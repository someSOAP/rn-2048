import { canMoveRight } from '../array'

describe('canMoveRight', () => {
  describe('two elements', () => {
    test('[0, 2]', () => {
      const result = canMoveRight([
  ...
  describe('one element', () => {
    it('0', () => {
      const result = canMoveRight([{ x: 0, y: 0, value: 0, merged: false }])

      expect(result).toBe(false)
    })
    it('2', () => {
      const result = canMoveRight([{ x: 0, y: 0, value: 2, merged: false }])
